...
export const updateUser = async (req, res, next) => {
  if (req.user.id !== req.params.userId) {
    return next(errorHandler(403, 'No tienes permiso para actualizar este usuario.'));
  }
  ...
  if (req.body.username) {
    if (req.body.username.length < 7 || req.body.username.length > 20) {
      return next(
        errorHandler(400, 'El nombre de usuario debe tener entre 7 y 20 caracteres.')
      );
    }
    ...
    if (!req.body.username.match(/^[a-zA-Z0-9]+$/)) {
      return next(
        errorHandler(400, 'Nombre de usuario solo puede contener letras y números.')
      );
    }
  }
  try {
...
export const deleteUser = async (req, res, next) => {
  if (!req.user.isAdmin && req.user.id !== req.params.userId) {
    return next(errorHandler(403, 'No tienes permiso para eliminar este usuario.'));
  }
  try {
...
export const signout = (req, res, next) => {
  try {
    res
      ...
      .status(200)
      .json('El usuario ha sido desconectado.');
  } catch (error) {
    next(error);
  }
};
